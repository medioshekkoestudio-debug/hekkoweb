...
tracking del cliente** hasta mucho después, o no aparecían del todo. En el panel del admin/estratega
sí se veían al instante. Parecía un problema de "admin vs estratega", pero **no lo era**: el guardado
...
➡️ Las vistas de **admin y estratega** leen la sesión (cookies) → **nunca** se cachean → siempre frescas.
...
- La versión actual de la caché es **`hekko-v1`**. Si se cambia la estrategia del SW,
...
- Las páginas **con login** (admin/estratega) ya son dinámicas por leer cookies; normalmente basta
...
- `src/app/tracking/[token]/page.tsx`: `dynamic = 'force-dynamic'` **+** `fetchCache = 'force-no-store'`.
  ...
  quién los subió (admin o estratega).
