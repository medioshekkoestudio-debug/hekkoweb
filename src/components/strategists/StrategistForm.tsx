...
import type { Strategist, CreateStrategistPayload } from '@/lib/types';
...
interface StrategistFormProps {
  onSaved: (strategist: Strategist) => void;
  onClose: () => void;
...
export default function StrategistForm({
  onSaved,
  onClose,
  strategist,
  focusPassword = false,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let res: Response;
    if (isEdit) {
      res = await fetch(`/api/strategists/${strategist!.id}`, {
        method: 'PATCH',
      ...
      const payload: CreateStrategistPayload = {
        full_name: form.full_name,
        email: form.email,
        password: form.password,
        phone: form.phone || undefined,
      };
      res = await fetch('/api/strategists', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) {
      ...
      setError(data.error || 'Error al guardar el estratega');
      return;
    }

    const saved: Strategist = await res.json();
    onSaved(saved);
  ...
  if (creds) {
    const waLink = buildWhatsAppLink(
      creds.phone,
      buildCredentialsMessage(creds.name, creds.email, creds.password, SITE_URL, companyName)
    );
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>
          Comparte estos datos de acceso con el estratega. La contraseña no se podrá
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {isEdit && focusPassword && (
        ...
        >
          Escribe una <strong>nueva contraseña</strong> y guárdala para reenviar el acceso al estratega por WhatsApp.
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <Button type="submit" variant="primary" fullWidth loading={loading}>
          {isEdit ? 'Guardar cambios' : 'Crear estratega'}
        </Button>
      </div>
    </form>
  );
